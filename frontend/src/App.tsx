...
import { getOverview, ingestMarketData, listDatasetCatalog, listDecisions, listExperiments, listPaperSessions, listResearchExperiments, reproduceExperiment, runResearchExperiment, startPaperSimulation } from "./api";
  ...
  return (
    <main className={isDark ? "app app--dark" : "app"}>
      ...
      <section className="section workspace" id="workspace"><div className="sectionHeading sectionHeading--row"><div><span className="kicker">YOUR WORKSPACE</span><h2>Use the real research tools</h2><p>Live records appear here when your local database has them.</p></div><span className="paperBadge">PAPER-ONLY</span></div><div className="workspaceGrid">
        ...
        <article className="workCard"><div className="workCardHeader"><div><span className="workIcon"><Gauge size={18} /></span><h3>PAPER sessions</h3></div><span>PAPER only</span></div>{paperSessions.loading ? <Loading label="Loading simulations" /> : paperSessions.error ? <ErrorState error={paperSessions.error} /> : paperSessions.data?.length ? <ul className="compactList">{paperSessions.data.map((session) => <li key={session.id}><span><strong>{session.symbol} · {humanize(session.strategy_name)}</strong><small>{session.status} · fills {String(session.metrics.fills ?? 0)} · equity {metric(session.metrics.ending_equity)}</small></span><span className="integrity">PAPER</span></li>)}</ul> : <EmptyState icon={<Gauge size={20} />} title="No PAPER sessions yet" text="Run a completed research result in PAPER to create a persisted replay." />}</article>
