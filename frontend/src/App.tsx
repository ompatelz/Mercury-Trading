...
import { attachResearchSource, getOverview, getPaperPerformanceHistory, getPaperSession, ingestMarketData, listDatasetCatalog, listDecisions, listExperiments, listPaperSessions, listResearchExperiments, reproduceExperiment, runResearchExperiment, startPaperSimulation } from "./api";
...
import { PaperPerformanceTimeline } from "./components/PaperPerformanceTimeline";
...
import type { DashboardOverview, DatasetCatalogItem, Decision, ExperimentListItem, PaperPortfolioHistoryPoint, PaperSessionDashboard, ReproductionResult } from "./types";
...
export function App() {
  ...
  const [paperPerformanceHistory, setPaperPerformanceHistory] = useState<Status<PaperPortfolioHistoryPoint[]>>({ loading: false });
  ...
  async function openPaperSession(sessionId: string) { setPaperDetail({ loading: true }); setPaperPerformanceHistory({ loading: true }); const results = await Promise.allSettled([getPaperSession(sessionId), getPaperPerformanceHistory(sessionId)]); settle(results[0], setPaperDetail); settle(results[1], setPaperPerformanceHistory); }
  ...
  return (
    <main className={isDark ? "app app--dark" : "app"}>
      ...
      <section className="section workspace" id="workspace"><div className="sectionHeading sectionHeading--row"><div><span className="kicker">YOUR WORKSPACE</span><h2>Use the real research tools</h2><p>Live records appear here when your local database has them.</p></div><span className="paperBadge">PAPER-ONLY</span></div><div className="workspaceGrid">
        ...
        <article className="workCard"><div className="workCardHeader"><div><span className="workIcon"><Gauge size={18} /></span><h3>PAPER sessions</h3></div><span>PAPER only</span></div>{paperSessions.loading ? <Loading label="Loading simulations" /> : paperSessions.error ? <ErrorState error={paperSessions.error} /> : paperSessions.data?.length ? <ul className="compactList">{paperSessions.data.map((session) => <li key={session.id}><button className="paperSessionButton" onClick={() => void openPaperSession(session.id)}><span><strong>{session.symbol} · {humanize(session.strategy_name)}</strong><small>{session.status} · fills {String(session.metrics.fills ?? 0)} · equity {metric(session.metrics.ending_equity)}</small></span><span className="integrity">PAPER</span></button></li>)}</ul> : <EmptyState icon={<Gauge size={20} />} title="No PAPER sessions yet" text="Run a completed research result in PAPER to create a persisted replay." />}{paperDetail.data && <PaperExecutionLedger session={paperDetail.data} />}{paperDetail.data && paperPerformanceHistory.data && <PaperSessionReport session={paperDetail.data} history={paperPerformanceHistory.data} />}{paperPerformanceHistory.loading && <Loading label="Loading persisted portfolio history" />}{paperPerformanceHistory.data && <PaperPerformanceTimeline points={paperPerformanceHistory.data} />}{paperPerformanceHistory.error && <ErrorState title="Couldn’t load PAPER performance history" error={paperPerformanceHistory.error} />}{paperDetail.error && <ErrorState title="Couldn’t load PAPER session" error={paperDetail.error} />}</article>
